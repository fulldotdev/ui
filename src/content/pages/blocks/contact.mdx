...
Each block renders a real form with named, required fields and a submit button.
Set `form.action` to your form handler and `form.method` to `"post"` (the
default) or `"get"`. The blocks do not provide a submission backend.
Pass `form.id` for a stable form ID; field IDs use it as their prefix.
