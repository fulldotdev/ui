---
type: blocks
title: Blocks
description: Full-width Astro block previews for hero, feature, FAQ, and call-to-action sections.
seo:
  title: Blocks
  description: Explore full-width Fulldev blocks rendered with Astro live previews and copyable source.
---

Blocks pages are for full sections rather than single UI primitives. The live
previews below render complete content blocks at a wider canvas so heroes,
feature grids, and call-to-action sections read like finished page slices
instead of tiny component demos.

## Hero

```astro live
---
import HomeHeroBlock from "@/components/blocks/hero-1.astro"
---

<HomeHeroBlock
  props={{
    title: "Build content-heavy Astro sites with composable sections",
    description:
      "Use the Fulldev block library when you want a complete hero, FAQ, or CTA section with layout, rhythm, and visual structure already solved.",
    actions: [
      { label: "Browse docs", href: "/docs/", variant: "default" },
      {
        label: "See components",
        href: "/docs/components/",
        variant: "outline",
      },
    ],
  }}
/>
```

## Feature Grid

```astro live
---
import {
  BlocksIcon,
  LayoutGridIcon,
  PaletteIcon,
  RocketIcon,
  SearchIcon,
  WandSparklesIcon,
} from "@lucide/astro"

import IconFeatureGridBlock from "@/components/blocks/features-2.astro"
---

<IconFeatureGridBlock
  props={{
    title: "Sections made for real content sites",
    description:
      "Each block handles layout, spacing, and presentation so you can compose full pages from larger pieces instead of wiring every section from scratch.",
    items: [
      {
        icon: BlocksIcon,
        title: "Complete sections",
        description:
          "Start from a full hero, CTA, FAQ, or footer instead of assembling a section one primitive at a time.",
      },
      {
        icon: LayoutGridIcon,
        title: "Responsive by default",
        description:
          "Blocks already account for page width, rhythm, and breakpoint behavior across editorial and marketing layouts.",
      },
      {
        icon: SearchIcon,
        title: "Easy to scan",
        description:
          "The wider preview surface makes it easier to judge spacing, density, and hierarchy before you ship a page.",
      },
      {
        icon: PaletteIcon,
        title: "Theme aware",
        description:
          "Blocks inherit tokens, surfaces, and typography so they stay coherent with the rest of the system.",
      },
      {
        icon: WandSparklesIcon,
        title: "Astro live previews",
        description:
          "Every example stays editable and copyable in MDX, including the entire section markup for each preview.",
      },
      {
        icon: RocketIcon,
        title: "Ready to compose",
        description:
          "Use blocks as starting points for landing pages, docs marketing surfaces, and product storytelling pages.",
      },
    ],
  }}
/>
```

## FAQ

```astro live
---
import FaqBlock from "@/components/blocks/faqs-1.astro"
---

<FaqBlock
  props={{
    title: "Questions teams ask before using blocks",
    description:
      "Blocks are meant for higher-level composition. Keep primitives in the component docs and use this area when you want full sections.",
    defaultItem: "blocks-faq-1",
    items: [
      {
        id: "blocks-faq-1",
        question: "When should I use a block page instead of a component doc?",
        answer:
          "Use a block page when the example needs to show an entire section with real spatial behavior, not just a single primitive in isolation.",
      },
      {
        id: "blocks-faq-2",
        question: "Why is the layout wider here?",
        answer:
          "A hero or FAQ section needs room to breathe. The wider container makes the preview feel like an actual page slice instead of a compressed demo.",
      },
      {
        id: "blocks-faq-3",
        question: "Does it still use Astro live code rendering?",
        answer:
          "Yes. These previews are still authored in MDX with `astro live`, so the full block source stays visible and copyable.",
      },
    ],
  }}
/>
```

## Call To Action

```astro live
---
import CallToActionBlock from "@/components/blocks/cta-1.astro"
---

<CallToActionBlock
  props={{
    title: "Ship fuller page sections without switching patterns",
    description:
      "The new Blocks layout keeps the docs authoring model, but the presentation is tuned for complete sections and larger visual slices.",
    actions: [
      { label: "Start with docs", href: "/docs/", variant: "default" },
      {
        label: "View components",
        href: "/docs/components/",
        variant: "outline",
      },
    ],
  }}
/>
```
