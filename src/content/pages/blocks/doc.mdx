...
<Doc1Block
  ...
  callout={{
    description: "Command menus work best when actions use short labels and predictable grouping.",
    button: {
      text: "View examples",
...
>
  ...
  <p>
    Add the command component from the registry, then import the pieces you need
    from the local component folder. The installed files include the root,
    input, list, group, item, separator, and empty state primitives.
  </p>
  <h2 id="anatomy">Anatomy</h2>
  <p>
    A command menu usually starts with a text input, followed by grouped results.
    Keep destructive actions visually separate from navigation and creation
    actions so keyboard users can move quickly without losing context.
  </p>
