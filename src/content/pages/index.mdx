---
presets: [base]

seo:
  title: Fulldev UI - Astro component and block library, open-source
  description: Make websites rapidly with pre-built components and blocks.
  image: og-image.png

title: Home

sections:
  - component: Hero
    size: lg
    tagline:
      We added layouts, now you can build entire sites
      # text: We added layouts, now you can build entire sites
      # href: /overview/installation/
    heading: The Astro UI library for content sites
    lead: Build content sites fast with components, blocks, layouts and full pages.
    buttons:
      - text: Get Started
        href: /overview/installation/
      - text: GitHub
        href: https://github.com/fulldotdev/ui
        icon: brand-github
  - component: Article
  - component: Content
    color: brand
    label: Blocks
    heading: Use pre-built blocks with just props
    paragraph: We created a great gallery of blocks to build content sites with a modern look and feel, all of our blocks are build with components from the library to achieve a consistent look.
    image: block-code.png
    buttons:
      - href: /blocks
        icon: layout
        text: View blocks

  - component: Content
    color: brand
    label: Components
    heading: Or build your own, with easy to use components
    paragraph: Our rich set of components make it easy to build your own blocks with great customizability through props and global theming.
    image: component-code.png
    reverse: odd
    buttons:
      - href: /base/badge
        icon: code
        text: View components

  - component: Cta
    size: lg
    variant: primary
    heading: Build content websites in a fraction of time
    lead: A library of components and blocks to build content sites, fast.
    buttons:
      - text: Get Started
        href: /overview/installation
        color: brand
      - text: Github
        href: https://github.com/fulldotdev/ui
        target: _blank
        icon: brand-github
        color: brand
---
