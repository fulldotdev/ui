...
matching `transition:persist` boundary retain their controllers.

Select, Tooltip, Hover Card and Navigation Menu keep closed content mounted by
default. Use their `mountStrategy="lazy"` prop to opt into Data Slot's detached
content behavior. See each component's content mounting notes before changing
custom scripts that query closed content.
