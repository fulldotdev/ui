...
Fulldev defaults `mountStrategy` to `"eager"`: closed content stays connected to
the document and hidden, preserving DOM queries and nested component setup.
Data Slot 1.x itself defaults to `"lazy"`, which detaches closed content.

Set `mountStrategy="lazy"` on the root to opt into detaching closed content.
Fulldev initializes nested Data Slot components inside retained content too.
Custom scripts using document queries should keep the eager strategy or wait
until the content opens. Both modes include the content in server-rendered
HTML; neither adds a no-JavaScript fallback.
