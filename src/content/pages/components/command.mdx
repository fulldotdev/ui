...
`CommandDialog` includes its own Dialog root. Put `DialogTrigger` in its `trigger`
slot; wrapping it in another `Dialog` creates an outer root with no content owned
by that root, which Data Slot 1.x rejects.
...
<CommandDialog title="Quick actions">
  <DialogTrigger slot="trigger" variant="outline">Quick actions</DialogTrigger>
  <Command>
    <CommandInput autofocus placeholder="Find an action..." />
    <CommandList>
...
Use native `autofocus` on a dialog's `CommandInput` so Data Slot places initial
focus in the search field. Leave it off standalone command menus.
...
<Command class="ring-foreground/10 w-full max-w-md ring-1">
  <CommandInput autofocus placeholder="Search shortcuts..." />
  <CommandList>
    <CommandEmpty>No results found.</CommandEmpty>
