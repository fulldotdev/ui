...
When you now call `getEntry('products', 'myproduct')` (or any of the other function, like `getCollection` or `getEntries`), it automatically returns a deepmerged version of the product and the layout. The entry itself allways has the highest priority, so you can override the layout values.
```markdown
---
checklist:
  - Free shipping
